/**
 * Test terminalGrowth against spreadsheet
 * 
 * This script tests how terminalGrowth affects valuation
...
console.log('🧪 Testing terminalGrowth Against Spreadsheet\n');
...
const baseInputs = {
  ...
  financial: {
    discountRate: 0.12,
    dilutionFactor: 0.15,
    terminalGrowth: 0.03 // Base value
  }
};
...
const testCases = [
  { name: 'Base (0.03)', terminalGrowth: 0.03, expected: 'match base' },
  { name: 'Lower (0.02)', terminalGrowth: 0.02, expected: 'lower than base' },
  { name: 'Higher (0.04)', terminalGrowth: 0.04, expected: 'higher than base' },
  { name: 'Much Higher (0.05)', terminalGrowth: 0.05, expected: 'much higher than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  if (index === 0) {
    ...
    // Higher terminalGrowth = higher terminal value = higher valuation
    // Lower terminalGrowth = lower terminal value = lower valuation
    if (testCase.terminalGrowth < 0.03 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.terminalGrowth > 0.03 && diff <= 0) {
      status = '❌';
      allPassed = false;
  ...
  results.push({
    name: testCase.name,
    terminalGrowth: testCase.terminalGrowth,
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower growth (0.02): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher growth (0.04): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Much higher (0.05): ${results[3].calculated.toFixed(2)}B (${((results[3].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);

console.log('\n' + '=' .repeat(70));

if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: terminalGrowth is correctly wired!');
  ...
  console.log('\n⚠️ RESULT: terminalGrowth wiring needs review');
