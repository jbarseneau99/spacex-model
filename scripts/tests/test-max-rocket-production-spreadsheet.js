/**
 * Test maxRocketProductionIncrease against spreadsheet
 * 
 * This script tests how maxRocketProductionIncrease affects valuation
...
console.log('🧪 Testing maxRocketProductionIncrease Against Spreadsheet\n');
...
const baseInputs = {
  earth: {
    ...
    maxRocketProductionIncrease: 0.25 // Base value
  },
  mars: {
...
// Test different maxRocketProductionIncrease values
const testCases = [
  { name: 'Base (0.25)', maxRocketProductionIncrease: 0.25, expected: 'match base' },
  { name: 'Lower (0.10)', maxRocketProductionIncrease: 0.10, expected: 'lower than base' },
  { name: 'Higher (0.50)', maxRocketProductionIncrease: 0.50, expected: 'higher than base' },
  { name: 'Much Higher (1.00)', maxRocketProductionIncrease: 1.00, expected: 'much higher than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  inputs.earth.maxRocketProductionIncrease = testCase.maxRocketProductionIncrease;
  ...
  if (index === 0) {
    ...
    // Higher max production increase = faster growth = higher revenue = higher valuation
    // Lower max production increase = slower growth = lower revenue = lower valuation
    if (testCase.maxRocketProductionIncrease < 0.25 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.maxRocketProductionIncrease > 0.25 && diff <= 0) {
      status = '❌';
      allPassed = false;
  ...
  results.push({
    name: testCase.name,
    maxRocketProductionIncrease: testCase.maxRocketProductionIncrease,
  ...
  console.log(`   maxRocketProductionIncrease: ${testCase.maxRocketProductionIncrease}`);
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower increase (0.10): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher increase (0.50): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Much higher (1.00): ${results[3].calculated.toFixed(2)}B (${((results[3].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);

console.log('\n' + '=' .repeat(70));

if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: maxRocketProductionIncrease is correctly wired!');
  console.log('   - Base case matches spreadsheet within tolerance');
  console.log('   - Parameter shows logical impact (higher increase = higher valuation)');
  console.log('   - Ready to move to next parameter\n');
  process.exit(0);
} else {
  console.log('\n⚠️ RESULT: maxRocketProductionIncrease wiring needs review');
