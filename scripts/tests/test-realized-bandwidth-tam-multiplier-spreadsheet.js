/**
 * Test realizedBandwidthTAMMultiplier against spreadsheet
 * 
 * This script tests how realizedBandwidthTAMMultiplier affects valuation
...
console.log('🧪 Testing realizedBandwidthTAMMultiplier Against Spreadsheet\n');
...
const baseInputs = {
  earth: {
    ...
    realizedBandwidthTAMMultiplier: 0.5 // Base value
  },
  mars: {
...
// Test different realizedBandwidthTAMMultiplier values
const testCases = [
  { name: 'Base (0.5)', realizedBandwidthTAMMultiplier: 0.5, expected: 'match base' },
  { name: 'Lower (0.3)', realizedBandwidthTAMMultiplier: 0.3, expected: 'lower than base' },
  { name: 'Higher (0.7)', realizedBandwidthTAMMultiplier: 0.7, expected: 'higher than base' },
  { name: 'Much Higher (1.0)', realizedBandwidthTAMMultiplier: 1.0, expected: 'much higher than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  inputs.earth.realizedBandwidthTAMMultiplier = testCase.realizedBandwidthTAMMultiplier;
  ...
  if (index === 0) {
    ...
    // Higher multiplier = more TAM realized = higher revenue = higher valuation
    // Lower multiplier = less TAM realized = lower revenue = lower valuation
    if (testCase.realizedBandwidthTAMMultiplier < 0.5 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.realizedBandwidthTAMMultiplier > 0.5 && diff <= 0) {
      status = '❌';
      allPassed = false;
  ...
  results.push({
    name: testCase.name,
    realizedBandwidthTAMMultiplier: testCase.realizedBandwidthTAMMultiplier,
  ...
  console.log(`   realizedBandwidthTAMMultiplier: ${testCase.realizedBandwidthTAMMultiplier}`);
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower multiplier (0.3): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher multiplier (0.7): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Much higher (1.0): ${results[3].calculated.toFixed(2)}B (${((results[3].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);

console.log('\n' + '=' .repeat(70));

if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: realizedBandwidthTAMMultiplier is correctly wired!');
  ...
  console.log('\n⚠️ RESULT: realizedBandwidthTAMMultiplier wiring needs review');
