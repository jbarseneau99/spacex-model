/**
 * Test wrightsLawLaunchCost against spreadsheet
 * 
 * This script tests how wrightsLawLaunchCost affects valuation
...
console.log('🧪 Testing wrightsLawLaunchCost Against Spreadsheet\n');
...
const baseInputs = {
  earth: {
    ...
    wrightsLawLaunchCost: 0.05 // Base value
  },
  mars: {
...
// Test different wrightsLawLaunchCost values
const testCases = [
  { name: 'Base (0.05)', wrightsLawLaunchCost: 0.05, expected: 'match base' },
  { name: 'Lower (0.02)', wrightsLawLaunchCost: 0.02, expected: 'higher costs than base' },
  { name: 'Higher (0.10)', wrightsLawLaunchCost: 0.10, expected: 'lower costs than base' },
  { name: 'Much Higher (0.15)', wrightsLawLaunchCost: 0.15, expected: 'much lower costs than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  inputs.earth.wrightsLawLaunchCost = testCase.wrightsLawLaunchCost;
  ...
  if (index === 0) {
    ...
    // Higher Wright's Law % = faster cost decline = lower costs = higher valuation
    // Lower Wright's Law % = slower cost decline = higher costs = lower valuation
    if (testCase.wrightsLawLaunchCost < 0.05 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base due to higher costs, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.wrightsLawLaunchCost > 0.05 && diff <= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be HIGHER than base due to lower costs, but is ${diff < 0 ? 'lower' : 'same'})`;
    } else {
  ...
  results.push({
    name: testCase.name,
    wrightsLawLaunchCost: testCase.wrightsLawLaunchCost,
  ...
  console.log(`   wrightsLawLaunchCost: ${testCase.wrightsLawLaunchCost}`);
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower Wright's Law (0.02): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher Wright's Law (0.10): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
...
if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: wrightsLawLaunchCost is correctly wired!');
  console.log('   - Base case matches spreadsheet within tolerance');
  console.log('   - Parameter shows logical impact (higher Wright\'s Law = higher valuation)');
  console.log('   - Ready to move to next parameter\n');
  process.exit(0);
} else {
  console.log('\n⚠️ RESULT: wrightsLawLaunchCost wiring needs review');
