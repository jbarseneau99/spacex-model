/**
 * Test irrThresholdEarthToMars against spreadsheet
 * 
 * This script tests how irrThresholdEarthToMars affects valuation
...
console.log('🧪 Testing irrThresholdEarthToMars Against Spreadsheet\n');
...
const baseInputs = {
  earth: {
    ...
    irrThresholdEarthToMars: 0.0 // Base value
  },
  mars: {
...
const testCases = [
  { name: 'Base (0.0)', irrThresholdEarthToMars: 0.0, expected: 'match base' },
  { name: 'Lower (-0.05)', irrThresholdEarthToMars: -0.05, expected: 'lower Earth value' },
  { name: 'Higher (0.05)', irrThresholdEarthToMars: 0.05, expected: 'higher Earth value' },
  { name: 'Much Higher (0.10)', irrThresholdEarthToMars: 0.10, expected: 'much higher Earth value' }
];
...
testCases.forEach((testCase, index) => {
  ...
  if (index === 0) {
    ...
    // Higher threshold = stay on Earth longer = higher Earth value
    // Lower threshold = switch to Mars earlier = lower Earth value
    if (testCase.irrThresholdEarthToMars < 0.0 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.irrThresholdEarthToMars > 0.0 && diff <= 0) {
      status = '❌';
      allPassed = false;
  ...
  results.push({
    name: testCase.name,
    irrThresholdEarthToMars: testCase.irrThresholdEarthToMars,
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower threshold (-0.05): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher threshold (0.05): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
...
if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: irrThresholdEarthToMars is correctly wired!');
  console.log('   - Base case matches spreadsheet within tolerance');
  console.log('   - Parameter shows logical impact (higher threshold = higher Earth valuation)');
  console.log('   - Ready to move to next parameter\n');
  process.exit(0);
} else {
  console.log('\n⚠️ RESULT: irrThresholdEarthToMars wiring needs review');
