/**
 * Test starshipLaunchesForStarlink against spreadsheet
 * 
 * This script tests how starshipLaunchesForStarlink affects valuation
...
console.log('🧪 Testing starshipLaunchesForStarlink Against Spreadsheet\n');
...
const baseInputs = {
  earth: {
    ...
    starshipLaunchesForStarlink: 0.9 // Base value
  },
  mars: {
...
// Test different starshipLaunchesForStarlink values
const testCases = [
  { name: 'Base (0.9)', starshipLaunchesForStarlink: 0.9, expected: 'match base' },
  { name: 'Lower (0.7)', starshipLaunchesForStarlink: 0.7, expected: 'lower than base' },
  { name: 'Higher (0.95)', starshipLaunchesForStarlink: 0.95, expected: 'higher than base' },
  { name: 'Much Higher (1.0)', starshipLaunchesForStarlink: 1.0, expected: 'much higher than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  inputs.earth.starshipLaunchesForStarlink = testCase.starshipLaunchesForStarlink;
  ...
  if (index === 0) {
    ...
    // Higher % = more launches for Starlink = higher Starlink capacity = higher revenue = higher valuation
    // Lower % = fewer launches for Starlink = lower Starlink capacity = lower revenue = lower valuation
    if (testCase.starshipLaunchesForStarlink < 0.9 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.starshipLaunchesForStarlink > 0.9 && diff <= 0) {
      status = '❌';
      allPassed = false;
  ...
  results.push({
    name: testCase.name,
    starshipLaunchesForStarlink: testCase.starshipLaunchesForStarlink,
  ...
  console.log(`   starshipLaunchesForStarlink: ${testCase.starshipLaunchesForStarlink}`);
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower % (0.7): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher % (0.95): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
...
if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: starshipLaunchesForStarlink is correctly wired!');
  ...
  console.log('\n⚠️ RESULT: starshipLaunchesForStarlink wiring needs review');
