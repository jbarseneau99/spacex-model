/**
 * Test wrightsLawTurnaroundTime against spreadsheet
 * 
 * This script tests how wrightsLawTurnaroundTime affects valuation
...
console.log('🧪 Testing wrightsLawTurnaroundTime Against Spreadsheet\n');
...
const baseInputs = {
  earth: {
    ...
    wrightsLawTurnaroundTime: 0.05 // Base value
  },
  mars: {
...
const testCases = [
  { name: 'Base (0.05)', wrightsLawTurnaroundTime: 0.05, expected: 'match base' },
  { name: 'Lower (0.02)', wrightsLawTurnaroundTime: 0.02, expected: 'lower than base' },
  { name: 'Higher (0.10)', wrightsLawTurnaroundTime: 0.10, expected: 'higher than base' },
  { name: 'Much Higher (0.15)', wrightsLawTurnaroundTime: 0.15, expected: 'much higher than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  if (index === 0) {
    ...
    // Higher Wright's Law % = faster turnaround = more launches = higher revenue = higher valuation
    // Lower Wright's Law % = slower turnaround = fewer launches = lower revenue = lower valuation
    if (testCase.wrightsLawTurnaroundTime < 0.05 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.wrightsLawTurnaroundTime > 0.05 && diff <= 0) {
      status = '❌';
      allPassed = false;
  ...
  results.push({
    name: testCase.name,
    wrightsLawTurnaroundTime: testCase.wrightsLawTurnaroundTime,
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower turnaround (0.02): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher turnaround (0.10): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
...
if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: wrightsLawTurnaroundTime is correctly wired!');
  ...
  console.log('\n⚠️ RESULT: wrightsLawTurnaroundTime wiring needs review');
