/**
 * Test optimusLearningRate against spreadsheet
 * 
 * This script tests how optimusLearningRate affects Mars valuation
...
console.log('🧪 Testing optimusLearningRate Against Spreadsheet\n');
...
const baseInputs = {
  ...
  mars: {
    ...
    optimusLearningRate: 0.05 // Base value
  },
  financial: {
    discountRate: 0.12,
    dilutionFactor: 0.15,
...
// Test different optimusLearningRate values
const testCases = [
  { name: 'Base (0.05)', optimusLearningRate: 0.05, expected: 'match base' },
  { name: 'Lower (0.02)', optimusLearningRate: 0.02, expected: 'lower than base' },
  { name: 'Higher (0.10)', optimusLearningRate: 0.10, expected: 'higher than base' },
  { name: 'Much Higher (0.15)', optimusLearningRate: 0.15, expected: 'much higher than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  inputs.mars.optimusLearningRate = testCase.optimusLearningRate;
  ...
  if (index === 0) {
    ...
    // Higher learning rate = faster productivity improvement = higher Mars value = higher valuation
    // Lower learning rate = slower productivity improvement = lower Mars value = lower valuation
    if (testCase.optimusLearningRate < 0.05 && diff >= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be LOWER than base, but is ${diff > 0 ? 'higher' : 'same'})`;
    } else if (testCase.optimusLearningRate > 0.05 && diff <= 0) {
      status = '❌';
      allPassed = false;
  ...
  results.push({
    name: testCase.name,
    optimusLearningRate: testCase.optimusLearningRate,
  ...
  console.log(`   optimusLearningRate: ${testCase.optimusLearningRate}`);
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower learning rate (0.02): ${results[1].calculated.toFixed(3)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher learning rate (0.10): ${results[2].calculated.toFixed(3)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
...
if (allPassed && results[1].calculated < results[0].calculated && results[2].calculated > results[0].calculated) {
  console.log('\n✅ RESULT: optimusLearningRate is correctly wired!');
  console.log('   - Base case matches spreadsheet within tolerance');
  console.log('   - Parameter shows logical impact (higher learning rate = higher valuation)');
  console.log('   - Ready to move to next parameter\n');
  process.exit(0);
} else {
  console.log('\n⚠️ RESULT: optimusLearningRate wiring needs review');
