/**
 * Test discountRate against spreadsheet
 * 
 * This script tests how discountRate affects valuation
...
console.log('🧪 Testing discountRate Against Spreadsheet\n');
...
// Test different discountRate values
const testCases = [
  { name: 'Base (0.12)', discountRate: 0.12, expected: 'match base' },
  { name: 'Lower (0.10)', discountRate: 0.10, expected: 'higher than base' },
  { name: 'Higher (0.15)', discountRate: 0.15, expected: 'lower than base' },
  { name: 'Much Higher (0.20)', discountRate: 0.20, expected: 'much lower than base' }
];
...
testCases.forEach((testCase, index) => {
  ...
  inputs.financial.discountRate = testCase.discountRate;
  ...
  if (index === 0) {
    ...
    // Higher discountRate = lower present value = lower valuation
    // Lower discountRate = higher present value = higher valuation
    if (testCase.discountRate < 0.12 && diff <= 0) {
      status = '❌';
      allPassed = false;
      note = ` (Should be HIGHER than base, but is ${diff > 0 ? 'higher' : 'lower'})`;
    } else if (testCase.discountRate > 0.12 && diff >= 0) {
      status = '❌';
      allPassed = false;
  ...
  console.log(`   discountRate: ${testCase.discountRate}`);
...
console.log(`\n   Parameter Impact:`);
console.log(`   - Lower rate (0.10): ${results[1].calculated.toFixed(2)}B (${((results[1].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Higher rate (0.15): ${results[2].calculated.toFixed(2)}B (${((results[2].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);
console.log(`   - Much higher (0.20): ${results[3].calculated.toFixed(2)}B (${((results[3].calculated - results[0].calculated) / results[0].calculated * 100).toFixed(2)}% vs base)`);

console.log('\n' + '=' .repeat(70));

if (allPassed && results[1].calculated > results[0].calculated && results[2].calculated < results[0].calculated) {
  console.log('\n✅ RESULT: discountRate is correctly wired!');
  console.log('   - Base case matches spreadsheet within tolerance');
  console.log('   - Parameter shows logical impact (lower rate = higher valuation)');
  console.log('   - Ready to move to next parameter\n');
  process.exit(0);
} else {
  console.log('\n⚠️ RESULT: discountRate wiring needs review');
