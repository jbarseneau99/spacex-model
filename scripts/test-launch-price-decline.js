/**
 * Test launchPriceDecline parameter wiring
 * 
 * This script tests how launchPriceDecline affects the valuation
...
console.log('🧪 Testing launchPriceDecline Parameter\n');
console.log('=' .repeat(60));

// Test different launchPriceDecline values
const testCases = [
  { name: 'Base (0.08)', launchPriceDecline: 0.08 },
  { name: 'Lower (0.05)', launchPriceDecline: 0.05 },
  { name: 'Higher (0.10)', launchPriceDecline: 0.10 },
  { name: 'Much Higher (0.15)', launchPriceDecline: 0.15 }
];
...
const baseInputs = {
  earth: {
    ...
    launchPriceDecline: 0.08 // Will be overridden
  },
  mars: {
...
testCases.forEach((testCase, index) => {
  ...
  if (baseSpreadsheetBillions && index === 0) {
    ...
    const baseCalculated = testCases[0].calculated || 123.67; // Use first test result
...
console.log('=' .repeat(60));
console.log('\n📝 Analysis:');
console.log('Currently launchPriceDecline is read but NOT used in calculations.');
console.log('We need to wire it up to affect launch costs (which affect total costs).\n');
console.log('Expected behavior:');
console.log('  - Lower launchPriceDecline = lower launch costs = higher valuation');
console.log('  - Higher launchPriceDecline = higher launch costs = lower valuation\n');
